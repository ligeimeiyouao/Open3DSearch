...
class PointPatchTransformer(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features):  # (1, 3, 10000)  (1, 6, 10000)
        self.sa.npoint = self.patches    # 384
        if self.training:
            self.sa.npoint -= self.patch_dropout
        centroids, feature = self.sa(xyz, features)   # (1, 3, 384)  (1, 256, 384)

        x = self.lift(torch.cat([centroids, feature], dim=1))   # (1, 384, 512)

        x = rst.supercat([self.cls_token, x], dim=-2)    # (1, 385, 512)
        centroids = rst.supercat([centroids.new_zeros(1), centroids], dim=-1)   # (1, 3, 385)

        centroid_delta = centroids.unsqueeze(-1) - centroids.unsqueeze(-2)    # (1, 3, 385, 385)
        x = self.transformer(x, centroid_delta)     # (1, 385, 512)
...
class Projected(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor, device=None, quantization_size=0.05):
        ...
        # case 2
        # tokens_feat = self.ppat(xyz.transpose(-1, -2).contiguous(), features.transpose(-1, -2).contiguous())
        # return tokens_feat
        # case 3
        # tmp, tokens_feat = self.ppat(xyz.transpose(-1, -2).contiguous(), features.transpose(-1, -2).contiguous())
        # feat = self.proj(tmp)
        # return feat, tokens_feat
        #####
